...
async function run() {
  try {
    ...
    if (process.platform === "win32") {
      PATH = OLD_PATH;
      while (PATH.indexOf("Git") != -1) PATH = PATH.replace("Git", "dummy");

      // Undo this otherwise gfortran libs are hidden
      PATH.replace(
        "C:\\Program Files\\dummy\\mingw64\\bin",
        "C:\\Program Files\\Git\\mingw64\\bin"
      );

      const BOOST_LIB = process.env.BOOST_ROOT + "\\lib";
      if (PATH.indexOf(BOOST_LIB) == -1) PATH = BOOST_LIB + ";" + PATH;
      ...
      if (compiler == "clang") {
        ...
        core.warning(
          "Compiler is set to " +
            compiler +
            " which is not recognized by this action"
        );
    }
